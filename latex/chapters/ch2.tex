...
% \subsection{Fundamentos y enfoques}

% La simulación de movilidad urbana combina, de forma general, tres bloques: representación de la red de transporte, modelado de la demanda de viajes y asignación de rutas bajo distintos supuestos operativos. Este tipo de herramientas se utiliza para evaluar escenarios de política pública, cambios en la oferta de transporte y efectos de nuevas infraestructuras antes de su despliegue real.

% En la práctica, los simuladores actuales suelen apoyarse en datos geoespaciales abiertos y en motores de enrutado especializados. Dos marcos ampliamente utilizados en investigación y planificación son MATSim y SUMO, que han consolidado metodologías reproducibles para experimentación en transporte a distintas escalas \cite{Horni2016MATSim,Lopez2018SUMO}.

% Este TFM adopta una aproximación aplicada: en lugar de desarrollar un motor de simulación desde cero, integra componentes consolidados (OSRM y OTP) sobre datos abiertos (OSM y GTFS), con una capa web orientada a análisis de escenarios.

% En esta línea, resulta especialmente relevante el enfoque de Hillel et al., donde la elección modal no se modela solo con el viaje observado, sino recreando el \textit{choice-set} disponible para cada desplazamiento (andar, bici, transporte público y coche) en el momento temporal correspondiente \cite{Hillel2018LPMC}. Este planteamiento de fusión de datos (encuesta de viajes + planificador de rutas + estimación de costes) es una base metodológica directa para proyectos que, como este TFM, combinan motor de rutas e IA de elección modal.

% La documentación técnica del caso LPMC también refuerza su interés práctico: el conjunto final recoge 81\,086 viajes, 31\,954 individuos y 17\,616 hogares en Londres, con variables de nivel de servicio por modo (duraciones segmentadas, transbordos y costes), además de variables sociodemográficas \cite{CSLPMC2019}. Esta estructura explica por qué LPMC se ha consolidado como referencia para evaluar modelos predictivos y comportamentales en elección modal.

% \subsubsection{Aplicaciones relevantes}

% En el ámbito urbano, las aplicaciones más habituales de estos sistemas incluyen:
% \begin{itemize}
%   \item comparación de tiempos y distancias entre modos para un mismo desplazamiento;
%   \item evaluación de accesibilidad bajo diferentes configuraciones de red;
%   \item análisis de cambios en oferta de transporte público (líneas, frecuencias y transbordos);
%   \item soporte a modelos de elección modal y análisis de impacto agregado.
% \end{itemize}

% La línea de trabajo de este proyecto se sitúa en este último punto: combinar métricas de red (rutas y tiempos) con un modelo de elección modal entrenado sobre datos observados (LPMC).
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figs/ch2_pipeline.png}
...
\begin{table}[ht]
    ...
    \begin{tabular}{|L{2.3cm}|L{2.9cm}|L{2.0cm}|L{2.3cm}|L{2.6cm}|}
        \hline
        \textbf{Solución} & \textbf{Tipo de red} & \textbf{Despliegue} & \textbf{Coste de uso} & \textbf{Adecuación al TFM} \\
        \hline
        OSRM & Red viaria OSM, perfiles por modo & Propio & Infraestructura propia & Muy adecuado para consultas masivas y control experimental \\
        \hline
        OTP & Red multimodal OSM + GTFS & Propio & Infraestructura propia & Muy adecuado para transporte público y análisis puerta a puerta \\
        \hline
        Google Maps Platform & API comercial multimodo gestionada & Tercero & Pago por petición y cuotas & Útil como referencia industrial, menos adecuado para reproducibilidad \\
        \hline
    \end{tabular}
\end{table}

Esta comparación resume la decisión tecnológica adoptada en el proyecto. Para un simulador académico orientado a experimentación, trazabilidad y ejecución repetida de escenarios, la combinación OSRM + OTP en infraestructura propia ofrece un equilibrio más adecuado que una API comercial gestionada, aunque implique una mayor complejidad de despliegue.

% \section{Síntesis y criterio de selección tecnológica}

% La revisión comparada permite distinguir dos estrategias: (i) consumo de plataformas gestionadas de terceros y (ii) operación de motores abiertos en infraestructura propia. Para un objetivo centrado en experimentación masiva y reproducibilidad, la segunda estrategia ofrece ventajas decisivas: control de versiones, congelación de datasets, trazabilidad de resultados y coste marginal más predecible al escalar consultas.

% En consecuencia, la arquitectura base del proyecto se fundamenta en OSRM + OTP en local para cálculo de red, manteniendo a Google Maps Platform y otras soluciones comerciales como referencia de comparación y posible vía de evolución futura orientada a producto.


% \section{Comparativa de soluciones}

% \subsection{OSRM frente a Google Maps Platform}

% Desde una perspectiva de ingeniería, la comparación entre OSRM y Google Maps Platform debe hacerse sobre criterios técnicos y operativos, no solo sobre calidad de ruta puntual.

% \subsubsection{Criterios de comparación}

% \textbf{Modelo de servicio}:
% \begin{itemize}
%   \item Google Maps Platform ofrece APIs gestionadas (por ejemplo, Routes API) \cite{GoogleRoutesAPI};
%   \item OSRM requiere operación propia, pero proporciona control total de infraestructura y datos \cite{OSRMDocs}.
% \end{itemize}

% \textbf{Coste y escalabilidad económica}:
% \begin{itemize}
%   \item en servicios comerciales el coste crece con el volumen de peticiones \cite{GoogleMapsPricing};
%   \item en despliegues propios el coste es principalmente de infraestructura y operación.
% \end{itemize}

% \textbf{Reproducibilidad y trazabilidad}:
% \begin{itemize}
%   \item con OSRM y OTP local se puede congelar dataset y configuración;
%   \item esto facilita replicar resultados entre iteraciones de experimentación.
% \end{itemize}

% \textbf{Conclusión aplicada al TFM}: para un simulador académico con necesidad de control y trazabilidad, OSRM + OTP en local es una elección coherente, aun asumiendo mayor complejidad de operación.
...
Este problema se ha abordado tradicionalmente mediante los denominados modelos de utilidad aleatoria (\textit{Random Utility Models}, RUM), ampliamente utilizados en transporte para analizar decisiones de elección discreta, como la elección del modo de transporte \cite[p.~14]{MartinBaos2023Thesis}. En este enfoque, el decisor elige una alternativa dentro de un conjunto de opciones mutuamente excluyentes, por ejemplo caminar, usar la bicicleta, utilizar transporte público o desplazarse en coche \cite{Train2009,McFadden1974}.

La idea central de estos modelos es que cada viajero asocia una utilidad a cada alternativa disponible dentro de su conjunto de elección, y se asume que elegirá aquella que le proporcione una mayor utilidad. Sin embargo, dicha utilidad no puede observarse de forma completa, ya que parte de los factores que influyen en la decisión sí pueden ser medidos por el analista, mientras que otros permanecen ocultos o no observados. Por ello, la utilidad de una alternativa suele descomponerse en un término determinista y un término aleatorio \cite[p.~14]{MartinBaos2023Thesis,Train2009}:
\begin{equation}
...
Bajo este planteamiento, el viajero se considera racional en el sentido de que selecciona la alternativa que maximiza su utilidad. En consecuencia, la elección observada puede expresarse en términos probabilísticos como la probabilidad de que la utilidad de una alternativa sea mayor o igual que la del resto de alternativas disponibles \cite[p.~15]{MartinBaos2023Thesis,Train2009}:
\begin{equation}
...
Las redes neuronales profundas (\textit{Deep Neural Networks}, DNN) constituyen otra familia de modelos de aprendizaje automático que ha experimentado un crecimiento significativo en los últimos años. Estas técnicas se basan en arquitecturas formadas por múltiples capas de neuronas artificiales que transforman progresivamente las variables de entrada mediante combinaciones lineales y funciones de activación no lineales \cite{Goodfellow2016,MartinBaos2023Thesis}.

A diferencia de los modelos basados en árboles, las redes neuronales permiten aprender representaciones internas de los datos a través de varias capas ocultas, lo que facilita capturar relaciones complejas entre variables. Esta capacidad de modelar interacciones no lineales ha motivado su aplicación en numerosos problemas de predicción y clasificación, incluyendo el análisis del comportamiento de los viajeros \cite{Goodfellow2016}.

En problemas de elección modal, las redes neuronales profundas pueden utilizarse para estimar la probabilidad de seleccionar cada modo de transporte a partir de atributos del desplazamiento y características sociodemográficas del usuario. Aunque estos modelos suelen requerir mayor volumen de datos y un proceso de ajuste más cuidadoso que los métodos basados en árboles, también ofrecen una elevada flexibilidad para capturar patrones complejos presentes en los datos \cite{MartinBaos2023TRC,MartinBaos2023Thesis}.
